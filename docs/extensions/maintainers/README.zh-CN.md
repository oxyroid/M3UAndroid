...
新增插件能力统一通过类型化契约和宿主 importer 接入，持久化始终位于宿主持有的数据 API 之后。
...
API major 不同、未知必要 capability、不支持的必要 hook schema 都应拒绝。未知可选 JSON 字段应忽略。runtime 从请求到结果始终保持类型化分发。
...
发现流程查询 `com.m3u.extension.action.BIND_EXTENSION`，再显式绑定解析出的 component；service 要求 `com.m3u.permission.BIND_EXTENSION_HOST`。外部代码留在插件 APK 进程中，通过 Android transport 通信。
...
插件边界上的凭据访问始终只使用 handle。

## 宿主持有的 Importer

解码之后，importer 是安全边界。它在写入前检查 stable reference、结果数量/大小、允许字段和数据所有权。

- Provider 订阅保存通用 provider/account 身份和 credential handle；UI/repository 根据该身份解析 provider。
