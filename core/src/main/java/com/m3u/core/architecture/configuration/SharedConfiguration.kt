...
class SharedConfiguration @Inject constructor(
    ...
    override val feedStrategy: MutableState<Int> =
        sharedPreferences.intAsState(DEFAULT_FEED_STRATEGY)
    override val useCommonUIMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_USE_COMMON_UI_MODE)

    override val rowCount: MutableState<Int> = sharedPreferences.intAsState(DEFAULT_ROW_COUNT)

    @ConnectTimeout
    override val connectTimeout: MutableState<Int> =
        sharedPreferences.intAsState(DEFAULT_CONNECT_TIMEOUT)
    override val godMode: MutableState<Boolean> = sharedPreferences.booleanAsState(DEFAULT_GOD_MODE)
    override val experimentalMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_EXPERIMENTAL_MODE)

    @ClipMode
    override val clipMode: MutableState<Int> = sharedPreferences.intAsState(DEFAULT_CLIP_MODE)
    override val scrollMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_SCROLL_MODE)
    override val autoRefresh: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_AUTO_REFRESH)
    override val isSSLVerification: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_SSL_VERIFICATION)
    override val fullInfoPlayer: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_FULL_INFO_PLAYER)
    override val initialTabIndex: MutableState<Int> =
        sharedPreferences.intAsState(DEFAULT_INITIAL_TAB_INDEX)
    override val noPictureMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_NO_PICTURE_MODE)
    override val silentMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_SILENT_MODE)
    override val cinemaMode: MutableState<Boolean> =
        sharedPreferences.booleanAsState(DEFAULT_CINEMA_MODE)
